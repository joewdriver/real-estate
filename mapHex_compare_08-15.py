...
df['bldg_price2008'] = data2008['total']

#print df
#getting rid of all of the zero values in square foot, so we don't divide by zero
df = df[df['square_foot'] !=0.0]

df['per_sq2008'] = df['bldg_price2008']/df['square_foot']
df['per_sq2015'] = df['bldg_price2015']/df['square_foot']

df['AV_TOTAL'] = df['per_sq2015'] - df['per_sq2008']
...
jenks_labels = ["Largest Loss"] + ['-40%', '-30%', '-20%', '-10%', "Even", '10%', '20%', '30%', '40%'] + ['Largest Increase']
...
cbar.set_label('quartiles', rotation=270)


##fig.suptitle("My location density in Seattle", fontdict={'size':24, 'fontweight':'bold'}, y=0.92)
ax.set_title("Boston Housing Change in Square Ft. Value Between 2008 and 2015", fontsize=14, y=0.98)

plt.savefig('./images/PerSq_norm_08-15.png', dpi=300, frameon=False, bbox_inches='tight', pad_inches=0.5, facecolor='#DEDEDE')
plt.show()
